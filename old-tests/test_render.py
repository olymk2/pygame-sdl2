...
def main():
    pygame-sdl2.init()
    pp.pprint(pygame-sdl2.render.get_drivers())
    pygame-sdl2.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), OPENGL)
    pygame-sdl2.display.set_caption("SDL2 render test")
    r = pygame-sdl2.render.Renderer(vsync=False)
    pp.pprint(r.info())
    ...
    while y < SCREEN_HEIGHT:
        while x < SCREEN_WIDTH:
            ...
            s = pygame-sdl2.render.Sprite(sprite_parts)
            s.pos = (x,y)
    ...
    clock = pygame-sdl2.time.Clock()

    sprites[0].color = (255,0,0)
    sprites[1].color = (100,100,255)

    con = pygame-sdl2.render.Container((-64,-64,32*10,32*10))
    for s in sprites:
        con.add(s)
