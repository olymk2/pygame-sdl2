...
# The version of pygame-sdl2. This should also be updated in version.py
VERSION="2.1.0"
...
def setup_env(name):
    # If PYGAME-SDL2_CC or PYGAME-SDL2_LD are in the environment, and CC or LD are not, use them.

    renpy_name = "PYGAME-SDL2_" + name
...
if android or ios:
    ...
    try:
        ...
        windeps = os.path.join(os.path.dirname(__file__), "pygame-sdl2_windeps")
...
cython("pygame-sdl2.event", libs=sdl_libs)
cython("pygame-sdl2.locals", libs=sdl_libs)
cython("pygame-sdl2.key", libs=sdl_libs)
cython("pygame-sdl2.mouse", libs=sdl_libs)
cython("pygame-sdl2.joystick", libs=sdl_libs)
cython("pygame-sdl2.pygame_time", libs=sdl_libs)
cython("pygame-sdl2.image", source=[ "src/write_jpeg.c", "src/write_png.c" ], libs=[ 'SDL2_image', "jpeg", png ] + sdl_libs)
cython("pygame-sdl2.transform", source=[ "src/SDL2_rotozoom.c" ], libs=sdl_libs)
cython("pygame-sdl2.gfxdraw", source=[ "src/SDL_gfxPrimitives.c" ], libs=sdl_libs)
...
if __name__ == "__main__":
    ...
    for i in temporary_package_data:
        os.unlink(os.path.join(os.path.dirname(__file__), "src", "pygame-sdl2", i))
