...
int Pygame-Sdl2_SavePNG(const char *file, SDL_Surface *surf,int compression){
	SDL_RWops *fp;
	int ret;
	...
	ret=Pygame-Sdl2_SavePNG_RW(fp,surf,compression);
	SDL_RWclose(fp);
	return ret;
}
...
int Pygame-Sdl2_SavePNG_RW(SDL_RWops *src, SDL_Surface *surf,int compression){
	png_structp png_ptr;
	png_infop info_ptr;
